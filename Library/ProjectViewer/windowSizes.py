...
def set_label_size(window,name,factor=1):
    ...
    if name == 'Mainwindow':
        window.ProjectNrBox.adjustSize()
        window.UserNrBox.adjustSize()
        window.ProjectNameBox.adjustSize()
        window.UserNameBox.adjustSize()
        h = window.UserNrBox.height()
        w = window.UserNameBox.width()
