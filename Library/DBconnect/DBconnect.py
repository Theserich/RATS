...
class DBconnect():
    def __init__(self):
        ...
        self.__myhost = sql_settings['myhost']
        self.__ok = True
        ...
        if self.__ok:
            try:
                ...
                if err.errno == 1045:
                    ...
                    self.__showLoginWindow(warning=f'unknown mysql.connector error: {err.errno}')
                if self.__ok:
    ...
    def __getCredentials(self):
        """Load username/password from system keyring."""
        # Username can be stored in settings or keyring meta
        self.__username = keyring.get_password("myapp-db", "username") or ""
        self.__password = keyring.get_password("myapp-db", self.__username) or ""

    def __storeCredentials(self):
        """Save username/password to system keyring."""
        if self.__remember:
            keyring.set_password("myapp-db", "username", self.__username)
            keyring.set_password("myapp-db", self.__username, self.__password)
